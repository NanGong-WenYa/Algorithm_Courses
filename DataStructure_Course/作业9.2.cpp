...
vector<Node*> RedBlackTree::dFind(Node* node, int key) {
	Node* current = node;
...
void RedBlackTree::dErase(Node* X, int key) {
	vector<Node*>nodes = dFind(X, key);
	int m = nodes.size();
	for (int i = 0; i < m; i++) {
		erase(X, key);
